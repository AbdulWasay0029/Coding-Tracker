...
export default async function DashboardPage() {
    ...
    return (
        <main className="max-w-6xl mx-auto px-4 py-12">
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-12 border-b border-border pb-8 animate-reveal stagger-1">
                <div className="flex items-center gap-6">
                    {session.user.image ? (
                        <Image src={session.user.image} alt="Avatar" width={80} height={80} className="rounded-full bg-surface border border-border" />
                    ) : (
                        <div className="w-20 h-20 rounded-full bg-surface border border-border flex items-center justify-center text-2xl font-bold text-white">
                            {session.user.name?.charAt(0)}
                        </div>
                    )}
                    <div>
                        <h1 className="text-4xl font-bold text-primary">{session.user.name}</h1>
                        <p className="text-text-secondary mt-1 font-mono text-sm">ID: {session.user.id}</p>
                    </div>
                </div>

                <div className="bg-surface border border-border px-6 py-4 rounded-xl shadow-sm flex items-center gap-4">
                    <div>
                        <h2 className="text-sm font-bold text-text-secondary uppercase tracking-wider">Total Solved</h2>
